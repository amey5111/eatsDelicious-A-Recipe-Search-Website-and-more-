      ...
      content: (
        <div className="flex">
          <div className="text-2xl px-8 py-5 font-sans font-medium">
            ...
            <div className="mt-6">{data.slides[2].content[3].children[0].text}</div>
          </div>
        </div>
      ),
    },
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="grid grid-cols-3 gap-8 w-full px-12">
            {data.facts.map((fact, index) => (
              <div key={index} className="bg-white bg-opacity-20 rounded-xl p-6 transform transition-all hover:scale-105 hover:bg-opacity-30">
                <h3 className="text-3xl font-bold mb-3 text-yellow-300">{fact.title}</h3>
                <p className="text-2xl">{fact.description}</p>
              </div>
            ))}
          </div>
